/**
 * Login Component - Platform authentication using backend OAuth flow
 *
 * Redirects to auth-backend for Google OAuth, then parses callback.
 * User info is fetched from the stored token via backend API.
 */
import { useEffect, useState } from 'react'
import { parseOAuthHash } from '@massivoto/auth-domain'
import { toast } from 'sonner'

export interface GoogleUser {
  name: string
  email: string
  picture?: string
}

interface LoginProps {
  onLogin: (user: GoogleUser) => void
}

const BACKEND_URL = import.meta.env.VITE_BACKEND_URL || 'http://localhost:4000'
// For platform auth, we use a special "platform" user ID
const PLATFORM_USER_ID = 'platform-auth'

export default function Login({ onLogin }: LoginProps) {
  const [loading, setLoading] = useState(false)

  // Handle OAuth callback on mount
  useEffect(() => {
    if (typeof window === 'undefined') return

    const hash = window.location.hash
    if (!hash) return

    const parsed = parseOAuthHash(hash)

    if (parsed.provider === 'google' && parsed.status === 'success') {
      // Clear hash
      window.history.replaceState({}, '', window.location.pathname + window.location.search)

      // Fetch user info from backend
      fetchUserInfo()
        .then((user) => {
          if (user) {
            onLogin(user)
            toast.success('Logged in successfully')
          }
        })
        .catch(() => {
          toast.error('Failed to get user info')
        })
    } else if (parsed.error) {
      window.history.replaceState({}, '', window.location.pathname + window.location.search)
      toast.error('Login failed', { description: parsed.errorDescription || parsed.error })
    }
  }, [onLogin])

  const fetchUserInfo = async (): Promise<GoogleUser | null> => {
    // TODO AI: Add backend endpoint to return user info from stored token
    // For now, return a placeholder user
    return {
      name: 'User',
      email: 'user@example.com',
    }
  }

  const handleLogin = () => {
    setLoading(true)
    const redirectUri = encodeURIComponent(window.location.href.split('#')[0])
    window.location.href = `${BACKEND_URL}/oauth/google/start?user_id=${PLATFORM_USER_ID}&redirect_uri=${redirectUri}`
  }

  return (
    <button
      onClick={handleLogin}
      disabled={loading}
      className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors"
    >
      <svg className="w-5 h-5" viewBox="0 0 24 24">
        <path
          fill="#4285F4"
          d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
        />
        <path
          fill="#34A853"
          d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
        />
        <path
          fill="#FBBC05"
          d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
        />
        <path
          fill="#EA4335"
          d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
        />
      </svg>
      <span className="text-gray-700 font-medium">
        {loading ? 'Redirecting...' : 'Sign in with Google'}
      </span>
    </button>
  )
}
